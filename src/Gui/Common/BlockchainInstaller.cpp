...
BlockchainInstaller::BlockchainInstaller(QObject* _parent) : QObject(_parent), m_blockIndexesFileName("blockindexes.dat"), m_blocksFileName("blocks.dat"),
  m_gadcoinDir(Settings::instance().getDataDir().absolutePath()), m_applicationDir(QDir::current()) {
}
...
void BlockchainInstaller::exec() {
  ...
  if (!checkIfgadcoinBlockchainExists()) {
    installBlockchain();
    return;
  }

  quint64 currentHeight;
  quint64 gadcoinHeight;
  if (!checkIfBlockchainOutdated(currentHeight, gadcoinHeight)) {
    return;
  }
  ...
  QuestionDialog dlg(tr("Blockchain installation"), QString(questionStringTemplate).arg(gadcoinHeight).arg(currentHeight), nullptr);
...
bool BlockchainInstaller::checkIfgadcoinBlockchainExists() const {
  return m_gadcoinDir.exists() && m_gadcoinDir.exists(m_blocksFileName);
}
...
bool BlockchainInstaller::checkIfBlockchainOutdated(quint64& _sourceHeight, quint64& _targetHeight) const {
  ...
  QFile targetBlockIndexesFile(m_gadcoinDir.absoluteFilePath(m_blockIndexesFileName));
...
QFileInfo BlockchainInstaller::gadcoinBlockchainInfo() const {
  return QFileInfo(m_gadcoinDir.absoluteFilePath(m_blocksFileName));
}
...
void BlockchainInstaller::installBlockchain() {
  ...
  Q_EMIT copyFileSignal(m_applicationDir.absoluteFilePath(m_blockIndexesFileName),
    m_gadcoinDir.absoluteFilePath(m_blockIndexesFileName));
  if (waitLoop.exec() != 0) {
    workerThread.quit();
    workerThread.wait();
    return;
  ...
  Q_EMIT copyFileSignal(m_applicationDir.absoluteFilePath(m_blocksFileName), m_gadcoinDir.absoluteFilePath(m_blocksFileName));
  if (waitLoop.exec() != 0) {
    workerThread.quit();
    workerThread.wait();
    return;
